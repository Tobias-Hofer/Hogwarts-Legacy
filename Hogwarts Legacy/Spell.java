...
public abstract class Spell {
	...
	public Spell(String name, int manaCost, MagicLevel levelNeeded) {
		...
		if (manaCost < 0) throw new IllegalArgumentException("Manakosten dürfen nicht Negativ sein");
		if (levelNeeded == null) throw new IllegalArgumentException("Gebrauchtes Level darf nicht Negativ sein");
	...
	public void cast(MagicSource source, MagicEffectRealization target) {
		if (source.provideMana(this.levelNeeded, manaCost))
			doEffect(target);
	}
	...
	public String toString() {
		return ("[" + this.name + "(" + this.levelNeeded + "): " + this.manaCost + " mana" + additionalOutputString() + "]");
	}
}